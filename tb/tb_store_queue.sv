...
module tb_store_queue;
  ...
  ren_uop_t execute_uop_i;
  logic [31:0] execute_addr_i, execute_data_i;
  logic [3:0] execute_strb_i;
  exec_wb_t execute_wb_o;
  logic commit_ready_o, commit_fire_i;
  ren_uop_t commit_uop_o;
  logic [31:0] commit_addr_o, commit_data_o;
  logic [3:0] commit_strb_o;
  logic load_query_valid_i;
  logic [31:0] load_query_addr_i;
  logic [RW:0] load_query_rob_i, rob_head_i;
  logic load_older_unknown_o;
  logic [3:0] load_forward_mask_o;
  logic [31:0] load_forward_data_o;
  ...
  task automatic execute_store(
    ...
    begin
      execute_valid_i = 1;
      execute_uop_i = '0;
      execute_uop_i.sq_idx = sq;
      execute_uop_i.rob_idx = rob;
      execute_addr_i = addr;
      execute_data_i = data;
      execute_strb_i = strb;
      #1;
      assert (execute_wb_o.rob.valid);
      @(posedge clk_i); #1;
      execute_valid_i = 0;
    end
  endtask
  ...
  initial begin
    ...
    execute_store(0, 0, DTIM_BASE + 32'h100, 32'h1122_3344, 4'hf);
    execute_store(1, 1, DTIM_BASE + 32'h100, 32'h0000_bbaa, 4'b0011);
    load_query_valid_i = 1;
    load_query_addr_i = DTIM_BASE + 32'h100;
    load_query_rob_i = 2;
    #1;
    assert (!load_older_unknown_o && load_forward_mask_o == 4'hf);
    assert (load_forward_data_o == 32'h1122_bbaa);
    load_query_rob_i = 4;
    #1;
    assert (load_older_unknown_o);
    ...
    execute_valid_i = 1;
    execute_uop_i.sq_idx = 2;
    execute_uop_i.rob_idx = 2;
    #1;
    assert (!execute_wb_o.rob.valid);
    ...
    load_query_valid_i = 1;
    load_query_addr_i = 32'h0200_0000;
    load_query_rob_i = 11;
    #1;
    assert (load_older_unknown_o && load_forward_mask_o == 0);
    flush_i = 1;
    #1;
